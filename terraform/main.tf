...
resource "aws_autoscaling_group" "catalogue" {
  name                      =  "${local.name}-${var.environment}"
  max_size                  = 10 # max number of instances you want to create instances
  min_size                  = 1  # min number of instances you want to create instances
  health_check_grace_period = 60 
  health_check_type         = "ELB"
  desired_capacity          = 2  # every time this  number of instances you want to keep standard
  target_group_arns = [aws_lb_target_group.catalogue.arn] # this will get the target groud with out adding the load balancer 
  launch_template {
